...
CouchDBDatabank.prototype.scan = function(type, onResult, onCompletion) {
    var self = this;
    if (!self._connected (onCompletion))
        return;

    // FIXME: refactor the common bits of scan and search.
    ...
    self.connection.tempView (map.join ("\n"), null, query, function (err, response) {
        if (err)
        {
            onCompletion(self._error (err, "scan", type));
        }

        response.rows.forEach (function (doc) { onResult(doc.data); });
