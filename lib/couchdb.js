...
/* cannot use "/" as separator:
   https://github.com/dominicbarnes/node-couchdb-api/pull/12  */
var SEPARATOR = ':';
    ...
    if (self.clear_database && !TEST_DATABASE_CLEARED)
    {
        step(
            ...
            function (err) {
                ...
                err ? this(err) : self.connection.create (this);
            },
            function () {
                onCompletion ();
            }
...
CouchDBDatabank.prototype.connect = function(params, onCompletion) {
    ...
    self.connection = couchdb.srv(self.location).db(self.database);
...
CouchDBDatabank.prototype.search = function(type, criteria, onResult, onCompletion) {
    ...
    // https://github.com/dominicbarnes/node-couchdb-api/issues/17
    var query_key = _(keys).map (function (key) { return criteria[key] });
    var query = { "qs": { "key": JSON.stringify (query_key) } };

    // FIXME: tempViews are slow.  automatically create permanent view
    // after N requests for the same view?

    self.connection.tempView (map.join ("\n"), null, null, query, function (err, response) {
        if (err)
        {
